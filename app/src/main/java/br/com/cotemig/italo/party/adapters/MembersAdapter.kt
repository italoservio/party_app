...
class MembersAdapter(var context: Context, var list: List<Members>) :
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MembersAdapter.MembersHolder {
    var view =
      LayoutInflater.from(context).inflate(R.layout.activity_members_list_item, parent, false)
  ...
  class MembersHolder(var view: View) : RecyclerView.ViewHolder(view) {
    fun bind(member: Members) {
      if (member.accepted == 1) {
        val name = view.findViewById<TextView>(R.id.member_name)
