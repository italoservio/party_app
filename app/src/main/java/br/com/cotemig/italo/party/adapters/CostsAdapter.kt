...
class CostsAdapter(var context: Context, var list: List<Cost>?) :
  RecyclerView.Adapter<CostsAdapter.CostsHolder>() {
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CostsAdapter.CostsHolder {
    var view =
      LayoutInflater.from(context).inflate(R.layout.activity_costs_list_item, parent, false)
  ...
  class CostsHolder(var view: View) : RecyclerView.ViewHolder(view) {
    fun bind(cost: Cost) {
      val price = view.findViewById<TextView>(R.id.cost_price)
      val description = view.findViewById<TextView>(R.id.cost_description)

      price.text = cost.price.toString()
      description.text = cost.description.toString()
    }
  }
}